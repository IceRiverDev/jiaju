                ...
                <div class="col align-self-center">
                    <div class="header-actions">
                        ...
                        <div class="header-bottom-set dropdown">
                            <a href="furnitureServlet?action=page&pageNo=1">后台管理</a>
                        </div>
                        <div class="header-bottom-set dropdown">
                            ...
                            <c:forEach items="${requestScope.page.items}" var="furn">
                                <tr>
                                    <td class="product-thumbnail">
                                        <a href="#"><img class="img-responsive ml-3" src="${furn.imagePath}"
                                                         alt=""/></a>
                                    </td>
                                    ...
                                    <td class="product-remove">
                                        <a href="furnitureServlet?action=showFurniture&id=${furn.id}&pageNo=${requestScope.page.pageNo}"><i
                                                class="icon-pencil"></i></a>
                                        <a class="deleteCss"
                                           href="furnitureServlet?action=delete&id=${furn.id}&pageNo=${requestScope.page.pageNo}"><i
        ...
        <div class="pro-pagination-style text-center mb-md-30px mb-lm-30px mt-6" data-aos="fade-up">
            <ul>
                ...
                <c:if test="${requestScope.page.pageNo > 1}">
                    <li><a href="furnitureServlet?action=page&pageNo=${requestScope.page.pageNo - 1}">上一页</a></li>
                </c:if>
                <c:choose>
                    <c:when test="${requestScope.page.pageTotalCount <= 5 || requestScope.page.pageNo <= 3}">
                    ...
                    <c:when test="${requestScope.page.pageTotalCount > 5}">
                        <c:choose>
                            <c:when test="${requestScope.page.pageNo > 3}">
                                ...
                                <c:choose>
                                    <c:when test="${requestScope.page.pageNo + 2 >= requestScope.page.pageTotalCount}">
                                        <c:set var="begin" value="${requestScope.page.pageTotalCount - 4}"/>
                                    </c:when>
                                </c:choose>

                                <c:choose>
                                    <c:when test="${end > requestScope.page.pageTotalCount}">
                ...
                <c:forEach begin="${begin}" end="${end}" var="i">
                    <c:choose>
                        <c:when test="${i == requestScope.page.pageNo}">
                            <li><a class="active"
                                   href="furnitureServlet?action=page&pageNo=${i}">${i}</a></li>
                        </c:when>
                        <c:otherwise>
                ...
                <c:if test="${requestScope.page.pageNo < requestScope.page.pageTotalCount}">
                    <li><a href="furnitureServlet?action=page&pageNo=${requestScope.page.pageNo + 1}">下一页</a></li>
                </c:if>
                <li><a href="furnitureServlet?action=page&pageNo=${requestScope.page.pageTotalCount}">最后一页</a></li>
